...
class QNetsimEngineRunner extends NetElementActivationRegistry implements Runnable, Callable<Boolean> {
	 ...
	 * cdobler, sep'14
	 */
	private final Queue<QNodeI> nodesQueue = new ConcurrentLinkedQueue<>(); //mads: Changed to QNodeI
	...
	private void moveNodes() {
		...
		this.lockNodes = true;
		QNodeI node;				//mads: Changed from QNodeImpl					
		Iterator<QNodeI> simNodes = this.nodesQueue.iterator(); //mads: Changed from QNodeImpl	
	 ...
	 */
	@Override
	protected void registerNodeAsActive(QNodeI node) {   //mads:  Changed from QNodeImpl
