 ...
 * @author dgrether
 */
public final class MyNetworkInverter {
	...
	public MyNetworkInverter(Network originalNet) {
		...
		for (Node node : this.originalNetwork.getNodes().values()) {
			for (Link inLink : node.getInLinks().values()) {
				for (Link outLink : node.getOutLinks().values()) {
						numberOfLinksGenerated = this.createInvertedLink(inLink, outLink, numberOfLinksGenerated, outLink.getAllowedModes());
				}
			}
		}
