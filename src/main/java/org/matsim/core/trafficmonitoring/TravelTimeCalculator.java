	...
	public TravelTime getLinkTravelTimes() {
		return new TravelTime() {
			...
			public double getLinkTravelTime(Link link, double time, Person person, Vehicle vehicle) {
				...
				if(vehicle != null){
					maximumVelocity = vehicle.getType().getMaximumVelocity();
				} else if(person != null && person.getAttributes().getAsMap().containsKey("v_0")){
					maximumVelocity = (double) person.getAttributes().getAttribute("v_0");		
				} 
					
				double minimumTravelTime = link.getLength() / maximumVelocity;
				
				double linkTTimeFromObservation = TravelTimeCalculator.this.getLinkTravelTime(link, time);
				return Math.max( minimumTravelTime, linkTTimeFromObservation) ;
			}
