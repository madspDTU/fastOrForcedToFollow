...
@Singleton
public class DesiredSpeedBicycleDijkstraFactory implements LeastCostPathCalculatorFactory {

	private final boolean usePreProcessData;
	private final Map<Network, PreProcessDijkstra> preProcessData = new HashMap<>();

	@Inject
	public DesiredSpeedBicycleDijkstraFactory() {
		this.usePreProcessData = false;
	}

	public DesiredSpeedBicycleDijkstraFactory(final boolean usePreProcessData) {
	...
	public synchronized LeastCostPathCalculator createDesiredSpeedPathCalculator(final Network network, final TravelDisutility travelCosts, final TravelTime travelTimes, String mode) {
		if (this.usePreProcessData) {
			PreProcessDijkstra preProcessDijkstra = this.preProcessData.get(network);
			if (preProcessDijkstra == null) {
				preProcessDijkstra = new PreProcessDijkstra();
