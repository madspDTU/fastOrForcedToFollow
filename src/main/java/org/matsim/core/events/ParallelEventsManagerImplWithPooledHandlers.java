 ...
 * @author cdobler
 */
public class ParallelEventsManagerImplWithPooledHandlers implements EventsManager {

	private final static Logger log = Logger.getLogger(ParallelEventsManagerImplWithPooledHandlers.class);
	...
	@Inject
	ParallelEventsManagerImplWithPooledHandlers(ParallelEventHandlingConfigGroup config) {
		this(config.getNumberOfThreads() != null ? config.getNumberOfThreads() : 1);
	}

    public ParallelEventsManagerImplWithPooledHandlers() {
		this(1);
	}
	
	public ParallelEventsManagerImplWithPooledHandlers(int numOfThreads) {
...
//		counter.set(0);
	...
	public void initProcessing() {
        ...
        ExceptionHandler uncaughtExceptionHandler = new ExceptionHandler(hadException, waitForEmptyQueuesBarrier, iterationEndBarrier);
		...
		for (int i = 0; i < numOfThreads; i++) {
			ProcessEventsRunnable processEventsRunnable = new ProcessEventsRunnable(eventsManagers[i], processedEventsChecker,
                    waitForEmptyQueuesBarrier, iterationEndBarrier, eventsQueues.get(i), eventsQueues.get(i + 1));
	...
	private static class ProcessEventsRunnable implements Runnable {
		...
		public ProcessEventsRunnable(EventsManager eventsManager, ProcessedEventsChecker processedEventsChecker, 
				CyclicBarrier waitForEmptyQueuesBarrier,CyclicBarrier iterationEndBarrier,
				Queue<Event> eventsQueue, Queue<Event> nextEventsQueue) {
		...
		public void run() {
			try {
				...
				while (true) {
				 ...
				 // Here (above) there used to be stuff handling lastEventOfSimStep
					{
						/*
	...
	private static class ExceptionHandler implements UncaughtExceptionHandler {
		...
		public ExceptionHandler(final AtomicReference<Throwable> hadException, CyclicBarrier waitForEmptyQueuesBarrier,
				CyclicBarrier iterationEndBarrier) {
