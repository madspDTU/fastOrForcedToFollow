 ...
 *
 */
public class FFFGenericPlanStrategyImpl<T extends BasicPlan, I> implements GenericPlanStrategy<T, I> {
	 ...
	 *
	 */
	public FFFGenericPlanStrategyImpl(final PlanSelector<T, I> planSelector) {
	...
	public void run(final HasPlansAndId<T, I> person) {
		...
		if (this.firstModule != null) {
			...
			if(!(plan instanceof FFFPlan)) {
				((Plan) plan).getPerson().removePlan((Plan) plan);
				plan = (T) new FFFPlan((Plan) plan, replanningContext.getIteration());
				((Plan) plan).getPerson().addPlan((Plan) plan);
				((Plan) plan).getPerson().setSelectedPlan((Plan) plan);
			}
		}
	}
