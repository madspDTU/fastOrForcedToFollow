	...
	private void handleCarDeparture(double now, MobsimDriverAgent agent, Id<Link> linkId) {
		...
		if (vehicle == null) {
			...
			if(agent.getMode().equals(TransportMode.bike)){
				linkId = Id.createLinkId(linkId.toString() + "_" + TransportMode.bike);
				qlink = (QLinkI) qNetsimEngine.getNetsimNetwork().getNetsimLink(linkId);
				vehicle = qlink.removeParkedVehicle(vehicleId);
				vehicle.setDriver(agent);
				agent.setVehicle(vehicle) ;
				
				agent.notifyArrivalOnLinkByNonNetworkMode(linkId);
				qlink.letVehicleDepart(vehicle);
			//mads ends
						
			} else if (vehicleBehavior == VehicleBehavior.teleport) {
