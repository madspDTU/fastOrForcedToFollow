...
// Change marked by //madsp. It makes sure that QNodeFFF's are also activated (before only QLinkImpls were). 
...
//			numOfRunners *= 10 ;
	...
	private void assignNetElementActivators() {
		...
		for (QNodeI node : network.getNetsimNodes().values()) {
			...
			} else if ( node instanceof QFFFNode ){
				((QFFFNode) node).setNetElementActivationRegistry(this.engines.get(i)); //mads:  added the QFFFNode instance..
			}
			nodes[i]++;
